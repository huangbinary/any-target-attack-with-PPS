...
class StegaStampEncoder(nn.Module):
    def __init__(self, img_size):
        ...
        self.secret_dense = Dense(100, 3 * img_size * img_size, activation='relu', kernel_initializer='he_normal')
    ...
    def forward(self, inputs):
        ...
        secrect = secrect.reshape(-1, 3, self.img_size, self.img_size)
...
class StegaStampDecoder(nn.Module):
    def __init__(self, img_size, secret_size=100):
        super(StegaStampDecoder, self).__init__()
        self.secret_size = secret_size
        self.stn = SpatialTransformerNetwork(img_size)
        in_features = (img_size // 2 ** 5) ** 2 * 128
        self.decoder = nn.Sequential(
            ...
            Flatten(),
            Dense(in_features, 512, activation='relu'),
...
def transform_net(encoded_image, args, global_step):
    img_size = encoded_image.size()[-1]
    ramp_fn = lambda ramp: np.min([global_step / ramp, 1.])

    rnd_bri = ramp_fn(args.rnd_bri_ramp) * args.rnd_bri
    rnd_hue = ramp_fn(args.rnd_hue_ramp) * args.rnd_hue
    rnd_brightness = get_rnd_brightness_torch(rnd_bri, rnd_hue, args.batch_size)  # [batch_size, 3, 1, 1]
    ...
    contrast_scale = contrast_scale.to(args.cuda)
    rnd_brightness = rnd_brightness.to(args.cuda)
    ...
    sat_weight = sat_weight.to(args.cuda)
    encoded_image_lum = torch.mean(encoded_image * sat_weight, dim=1).unsqueeze_(1)
    encoded_image = (1 - rnd_sat) * encoded_image + rnd_sat * encoded_image_lum

    # jpeg
    encoded_image = encoded_image.reshape([-1, 3, img_size, img_size])
    if not args.no_jpeg:
...
def build_model(encoder, decoder, discriminator, lpips_fn, secret_input, image_input, l2_edge_gain,
    ...
    img_size = image_input.size()[-1]
    input_warped = torchgeometry.warp_perspective(image_input, M[:, 1, :, :], dsize=(img_size, img_size),
                                                  flags='bilinear')
    mask_warped = torchgeometry.warp_perspective(torch.ones_like(input_warped), M[:, 1, :, :],
                                                 dsize=(img_size, img_size), flags='bilinear')
    ...
    residual = torchgeometry.warp_perspective(residual_warped, M[:, 0, :, :], dsize=(img_size, img_size),
                                              flags='bilinear')
    ...
    if borders == 'no_edge':
        ...
        encoded_image = torchgeometry.warp_perspective(encoded_image, M[:, 0, :, :], dsize=(img_size, img_size),
                                                       flags='bilinear')
        input_unwarped = torchgeometry.warp_perspective(image_input, M[:, 0, :, :], dsize=(img_size, img_size),
                                                        flags='bilinear')
    elif borders.startswith('random'):
        mask = torchgeometry.warp_perspective(torch.ones_like(residual), M[:, 0, :, :], dsize=(img_size, img_size),
                                              flags='bilinear')
        encoded_image = residual_warped + input_unwarped
        encoded_image = torchgeometry.warp_perspective(encoded_image, M[:, 0, :, :], dsize=(img_size, img_size),
                                                       flags='bilinear')
        input_unwarped = torchgeometry.warp_perspective(input_warped, M[:, 0, :, :], dsize=(img_size, img_size),
                                                        flags='bilinear')
    ...
    cross_entropy = cross_entropy.to(args.cuda)
    ...
    falloff_im = falloff_im.to(args.cuda)
    ...
    yuv_scales = yuv_scales.to(args.cuda)
